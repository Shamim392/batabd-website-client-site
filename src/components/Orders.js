...
const Orders = () => {
  ...
  useEffect(() => {
    fetch(`https://agile-island-82672.herokuapp.com/orders`)
      .then((res) => res.json())
      .then((data) => {
        setOrders(data);
        setLoading(false);
  ...
  const handleStatusChange = (id, status) => {
    ...
    fetch("https://agile-island-82672.herokuapp.com/updateOrderStatus", {
      method: "put",
  ...
  const deletion = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://agile-island-82672.herokuapp.com/placeorder/${id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
